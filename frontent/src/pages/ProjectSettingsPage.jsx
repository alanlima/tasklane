...
function ProjectLifecycle({ project, onGetArchiveSummary, onArchive, onRestore, onDelete }) {
  ...
  const archive = async () => { setBusy(true); setError(""); try { await onArchive(Boolean(summary?.incomplete_tasks)); } catch { try { setSummary(await onGetArchiveSummary()); setError("Could not archive this project. Review the updated task counts and try again."); } catch { setError("Could not archive this project or refresh its task counts. Cancel and try again."); setMode(""); } } finally { setBusy(false); } };
  const restore = async () => { setBusy(true); setError(""); try { await onRestore(); } catch { setError("Could not restore this project. Please try again."); } finally { setBusy(false); } };
