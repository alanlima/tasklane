...
export function useTasklane() {
  ...
  const archiveProject = useCallback(async () => { if (!project) return; await api.archiveProject(project.id, true); setProject(null); await refreshProjects(); }, [project, refreshProjects]);
  const restoreProject = useCallback(async () => { if (!project) return; const restored = await api.restoreProject(project.id); setProject((current) => current ? { ...current, ...restored } : current); await refreshProjects(); }, [project, refreshProjects]);
  const deleteProject = useCallback(async (confirmationName) => { if (!project) return; await api.deleteProject(project.id, confirmationName); setProject(null); await refreshProjects(); }, [project, refreshProjects]);
